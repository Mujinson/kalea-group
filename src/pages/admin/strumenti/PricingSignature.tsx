...
export default function PricingSignature() {
  const { settings, update } = useBrandPricingRule("signature", SCONTI, defaults);
  ...
  return (
    <div style={{ fontFamily: "'new-order', sans-serif", color: "#1A1A1A", maxWidth: 1200, margin: "0 auto", padding: "8px 4px" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "#64748B" }}>Woodco · Parquet premium · Spine 45/52/Corta · Esagono · Pattern · Listello · Quadrotte · Arrow · Rovere Recupero · Listino Aprile 2026</p>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #E6E9F0", borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          <div>
            <div style={{ fontSize: 13, color: "#475569", marginBottom: 10 }}>Sconto fornitore Woodco Signature</div>
      ...
      <div style={{ background: "#fff", border: "1px solid #E6E9F0", borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cerca..."
            style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid #E6E9F0", fontSize: 13, outline: "none", width: 180, background: "#F8FAFC" }} />
          {CATEGORIE.map(c => (
        ...
        <div style={{ maxHeight: 460, overflowY: "auto", borderRadius: 8, border: "1px solid #E6E9F0" }}>
              ...
              {pagedRows.map(p => {
                ...
                return (
                  <tr key={p.id} onClick={() => setSelectedId(p.id)}
                    ...
                    <td style={{ padding: "9px 10px", borderBottom: "0.5px solid #E6E9F0", fontWeight: 500, maxWidth: 200 }}>{p.nome}</td>
                    <td style={{ padding: "9px 10px", borderBottom: "0.5px solid #E6E9F0", fontSize: 11, color: "#64748B", whiteSpace: "nowrap" }}>{p.dims}</td>
                    ...
                    <td style={{ padding: "9px 10px", borderBottom: "0.5px solid #E6E9F0", color: "#7B3A10", fontWeight: 500, whiteSpace: "nowrap" }}>{fmt2(prezzo)}/mq</td>
                    ...
                    <td style={{ padding: "9px 10px", borderBottom: "0.5px solid #E6E9F0", whiteSpace: "nowrap" }}>
                      <button onClick={e => { e.stopPropagation(); setSelectedId(p.id); }}
                        style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid", cursor: "pointer", fontSize: 11,
                          background: isSel ? "#7B3A10" : "transparent", color: isSel ? "#fff" : "#7B3A10", borderColor: isSel ? "#7B3A10" : "#E6E9F0" }}>
                        {isSel ? "✓" : "Usa"}
                      </button>
        ...
        <div style={{ fontSize: 11, color: "#64748B", marginTop: 8 }}>{pagedRows.length} di {totalRows} articoli · Prezzi IVA esclusa · Listino Signature</div>
      ...
      {selected && prev && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #E6E9F0", borderRadius: 12, padding: "20px 24px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", fontSize: 13 }}>
              <span style={{ color: "#475569" }}>Break-even (costo acquisto)</span>
