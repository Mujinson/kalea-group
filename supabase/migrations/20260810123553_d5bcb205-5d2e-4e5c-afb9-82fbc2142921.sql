...
BEGIN
  ...
  THEN
    v_raw := COALESCE(NEW.created_by, NEW.assigned_to);
    IF v_raw IS NULL OR btrim(v_raw) = '' THEN RETURN NEW; END IF;

    IF v_raw ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
      v_user_id := v_raw::uuid;
    ELSE
      SELECT id INTO v_user_id FROM auth.users WHERE lower(email) = lower(btrim(v_raw)) LIMIT 1;
    END IF;
